...
const DIFFICULTY_CONNECT = 18;   // ~262K hashes, ~1-2s on modern hardware
const DIFFICULTY_ACCOUNT = 20;   // ~1M hashes, ~2-5s on modern hardware
...
function generateChallenge(type) {
  const difficulty = type === 'account' ? DIFFICULTY_ACCOUNT : DIFFICULTY_CONNECT;
...
function verify(challenge, nonce) {
  ...
  if (!hasLeadingZeros(hash, issued.difficulty)) {

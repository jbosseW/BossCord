...
module.exports = {
  init(io, socket, deps) {
    var { user, socketAccountMap, accounts, cords, filter, pow, checkEventRate, validateUrl, state } = deps;
    ...
    socket.on('account_set_pin', async (data) => {
      try {
        if (!checkEventRate(socket, 'account_set_pin', 5, 60000)) {
          socket.emit('error', { message: 'Too many attempts' });
          return;
        }
    ...
    socket.on('account_change_pin', async (data) => {
      try {
        if (!checkEventRate(socket, 'account_change_pin', 3, 60000)) {
          socket.emit('error', { message: 'Too many attempts' });
          return;
        }
        var key = socketAccountMap.get(socket.id);
    ...
    socket.on('account_delete', async (data) => {
      try {
        ...
        if (!checkEventRate(socket, 'account_delete', 3, 60000)) return;
        ...
        // Cascade delete: remove all cords by this account
        const cordsRemoved = cords.deleteByAccount(key);
        accounts.deleteAccount(key);
        socketAccountMap.delete(socket.id);
        socket.emit('account_deleted');
        console.log(`[account] ${user.name} deleted their account (removed ${cordsRemoved} cords)`);
      } catch (err) {
    ...
    socket.on('get_user_profile', (data) => {
      try {
        ...
        // Rate limit: 10 profile lookups per minute
        if (!checkEventRate(socket, 'get_user_profile', 10, 60000)) return;
